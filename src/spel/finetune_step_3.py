...
from spel.data_loader import get_dataset, get_dataset_aida
...
FINETUNE_MULTI_GPU = True

class FinetuneS3(SpELAnnotator):
    def __init__(self):
        super(FinetuneS3, self).__init__()

    def finetune_new(self, checkpoint_name, n_epochs, batch_size, bert_dropout=0.2, encoder_lr=5e-5, label_size=8196,
                     accumulate_batch_gradients=4,  exclude_parameter_names_regex='embeddings|encoder\\.layer\\.[0-2]\\.',
                     eval_batch_size=1):
        # device = torch.device('cuda', 1)
        self.init_desc_model_from_scratch(device=device)
        ...
        #     self.bert_out = nn.DataParallel(self.bert_out).to(device)
        if bert_dropout > 0:
            for m in self.bert_lm.modules():
                if isinstance(m, torch.nn.Dropout):
                    m.p = bert_dropout
            for m in self.bert_out.modules():
                if isinstance(m, torch.nn.Dropout):
                    m.p = bert_dropout
        optimizers = self.create_desc_optimizers(encoder_lr, 0.0, exclude_parameter_names_regex)
        criterion = nn.BCEWithLogitsLoss()
        best_f1 = 0.0
        for epoch in range(n_epochs):
            print(f"Beginning fine-tune epoch {epoch} ...")
            _iter_ = tqdm(enumerate(
                get_dataset_aida(split='train', batch_size=batch_size, label_size=label_size,
                            get_labels_with_high_model_score=self.get_highest_confidence_model_predictions)
            ))
            self.create_dict_after_getdata()
            total_loss = 0
            cnt_loss = 0
            ...
            sprecision, srecall, sf1, sf05, snum_proposed, snum_correct, snum_gold, subword_eval = self.evaluate_new(
